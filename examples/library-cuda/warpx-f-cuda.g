...
prefix := "warpx_f_cuda";     ##  PICKME #define CUDAFUNCNAME warpx_cuda
...
t := let(name := prefix,
    ...
    TFCallF(
        ...
        rec(XType := TPtr(TArrayNDF(TReal, [n, n, n])), YType := TPtr(TArrayNDF(TReal, [n, n, n])), 
...
if IsBound(opts) then
    opts.includes := [ "\"CudaWarpXConst.h\"" ];
fi;
